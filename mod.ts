...
interface ElementSelf<Message> extends Element {
  dispatch(msg: Message): void;
}

export default <State, Message extends [string, unknown]>(
  name: string,
  template: (self: ElementSelf<Message>, state: State) => string,
  initialState: (self: ElementSelf<Message>) => State,
  update: (self: ElementSelf<Message>, state: State, message: Message) => State,
  observedAttributes: [
    string,
  ...
  const elementClass = class
    extends HTMLElement
    implements ElementSelf<Message>
  {
